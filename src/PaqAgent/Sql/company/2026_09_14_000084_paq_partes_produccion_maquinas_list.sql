CREATE OR ALTER PROCEDURE dbo.PAQ_PartesProduccion_MaquinasList
...
BEGIN
    ...
    DECLARE @SortCol NVARCHAR(50) = UPPER(LTRIM(RTRIM(ISNULL(NULLIF(@Sort, N''), N'CODIGO_MAQUINA'))));
    DECLARE @DirNorm NVARCHAR(4) = CASE WHEN UPPER(LTRIM(RTRIM(ISNULL(@Dir, N'ASC')))) = N'DESC' THEN N'DESC' ELSE N'ASC' END;

    IF @SortCol NOT IN (N'CODIGO_MAQUINA', N'NOMBRE', N'ACTIVA', N'ID_MAQUINA')
        SET @SortCol = N'CODIGO_MAQUINA';
    ...
    WHERE (@FilterActiva IS NULL OR ACTIVA = @FilterActiva)
      AND (@FilterCodigo IS NULL OR CODIGO_MAQUINA LIKE N''%'' + @FilterCodigo + N''%'')
