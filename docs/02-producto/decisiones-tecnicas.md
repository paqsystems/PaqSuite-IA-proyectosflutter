...
- Tabla `empresas_conexion` guarda `agent_id`, `client_id` y el **hash** del token (o el token cifrado con `APP_KEY`, igual que hoy `password` SQL). **Default scaffold (H3):** columnas en `empresas_conexion`; **sin tabla `agents`** en el MVP.
...
|------|----------------|-----------|
| `PaqSuite-IA-AgenteCliente-PAQ` (este) | `sdd-reformulacion` | Agente, Gateway, instalador, docs SDD (`src/` + `docs/`) |
| `PaqSuite-IA-TANGO` | rama equivalente | Contrato `empresas_conexion` + `AgentGatewayClient` + quitar fallback. **TR-001 / TR-007 / TR-008 viven aquí.** |
...
- Online ⇔ `last_seen_at` dentro del TTL configurado. **Default scaffold (H8):** heartbeat **30 s**, TTL **90 s** (`PaqContracts.AgentDefaults`).
- **Autoridad del online: Gateway** (H4). Laravel consulta el status API; no es obligatorio persistir `last_seen_at` en Laravel en el MVP.
...
## D18 — Defaults de scaffold (informe 08, 2026-09-04)

Aplicados porque el humano no los fijó antes. Si hay que cambiarlos, se actualiza **este** documento primero.
