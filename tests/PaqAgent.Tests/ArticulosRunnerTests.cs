...
public class ArticulosBuscarRunnerTests
{
    [Fact]
    public async Task RunAsync_withoutSqlConfig_returnsDegraded()
    {
        var runner = new ArticulosBuscarRunner(new FakeBuscarExecutor((_, _, _) => throw new InvalidOperationException("no call")));
    ...
    public async Task RunAsync_missingDatabase_returnsInvalidParameters()
    {
        ...
        var outcome = await runner.RunAsync(
            LabOptionsWithSql(),
            new Dictionary<string, object?> { ["texto"] = "TOR" },
            30,
            CancellationToken.None);
    ...
    {
        var runner = new ArticulosBuscarRunner(new FakeBuscarExecutor((_, _, _) =>
        ...
        Assert.Single(list);
        Assert.Equal("ART01", list[0]["codigo"]);
    }
    ...
    private sealed class FakeBuscarExecutor : IArticulosBuscarSpExecutor
    {
...
public class ArticulosObtenerRunnerTests
{
    [Fact]
    public async Task RunAsync_notFound_returnsNotFound()
    {
        var runner = new ArticulosObtenerRunner(new FakeObtenerExecutor((_, _) => null));
    ...
    {
        var runner = new ArticulosObtenerRunner(new FakeObtenerExecutor((_, _) =>
        ...
        Assert.Equal("Tornillo", row["descripcion"]);
    }
    ...
    private sealed class FakeObtenerExecutor : IArticulosObtenerSpExecutor
    {
